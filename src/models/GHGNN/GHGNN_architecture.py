...
try:  # pragma: no cover
    from ..utilities.mol2graph import cat_dummy_graph  # type: ignore
    ...
    def forward(self, system_graph):
        ...
        for _ in range(self.num_step_message_passing):
            if torch.cuda.is_available():
                ...
                node_feats = F.gelu(self.gnn_layer(x=node_feats.type(torch.FloatTensor).cuda(),
                                                   edge_index=edge_index.type(torch.LongTensor).cuda(),
                                                   edge_attr=edge_feats.type(torch.FloatTensor).cuda()))
            else:
                node_feats = F.gelu(self.gnn_layer(x=node_feats.type(torch.FloatTensor),
...
'''x-节点特征矩阵:[节点数量,节点特征数]-[1186,38]
        edge_index-图的连接性(键的起点和终点的索引):[2,边的数量]-[2,2266]
        edge_attr-边特征矩阵(化学键的特征):[边的数量,每条边的特征维度]-[2266,38]
        src-来源节点特征
        dst-目标节点特征'''
...
class GHGNN(nn.Module):#GHGNN的主要模块:两个图神经网络，两次全局池化，两个多层感知器
    def __init__(self, v_in, e_in, u_in, hidden_dim):
    ...
    def forward(self, solvent, solute, T, scaler=None, ln_gamma=True):
        ...
        # Molecular descriptors based on MOSCED model-定义分子图的全局特征
        ...
        # 直接使用 Gibbs-Helmholtz 方程: output = A + B/T
        # 移除了不适用于Brouwer_2021数据集的归一化和温度转换逻辑
        output = A + B / T
